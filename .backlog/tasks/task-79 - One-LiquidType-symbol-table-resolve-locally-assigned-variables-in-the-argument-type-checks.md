...
A `LiquidType` symbol table already exists privately inside `InvalidWriteTarget`. Promote it to a shared, per-file, memoized module (`src/variable-types.ts`), give it the scope handling `shape-analysis.ts` already proved (branch scoping, loop shadowing, unparseable-markup invalidation), and have all four checks read it.
<!-- SECTION:DESCRIPTION:END -->

## Acceptance Criteria
<!-- AC:BEGIN -->
- [x] #1 `{% assign x = 403 %}{{ x | t }}` reports the same message `{{ 403 | t }}` reports, with a control asserting `{% assign x = 'a' %}{{ x | t }}` stays silent
- [x] #2 One LiquidType table in check-common, consumed by ValidFilterArgumentTypes, ValidTagArgumentTypes, ValidRenderPartialArgumentTypes and InvalidWriteTarget — the private tracker in InvalidWriteTarget is deleted, not duplicated
