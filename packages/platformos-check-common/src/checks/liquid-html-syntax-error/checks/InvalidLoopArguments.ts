 ...
 *
 * `positional` NARROWS the answer and never widens it: a tag parameter that says nothing about
 * how it is written matches either spelling. `tags.json` publishes `positional` on no tag
 * parameter at all, so requiring equality compared `undefined === false` and answered
 * "unsupported" for every argument of every loop — `{% for x in y limit: 2 %}` was reported as
 * an unknown argument, with an autofix that DELETED it. Absence is not evidence, and a syntax
 * error blocks a write.
 */
