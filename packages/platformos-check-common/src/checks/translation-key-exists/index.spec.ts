...
{{"nested.key" | t}}`,
  ...
  it('should not report offense when translation key has a default filter argument', async () => {
    const offenses = await check(
      {
        'code.liquid': `{{ 'modules/community/app.at' | t: default: 'at' }}`,
      },
      [TranslationKeyExists],
    );
  ...
  it('should still report offense when t filter has args but no default', async () => {
    const offenses = await check(
      {
        'code.liquid': `{{ 'missing.key' | t: name: 'John' }}`,
      },
      [TranslationKeyExists],
    );
    expect(offenses).to.have.length(1);
