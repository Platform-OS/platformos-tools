...
import { makeGetDefaultLocaleFileUri, makeGetDefaultTranslations } from './context-utils';
...
/**
 * The memoization on `makeGetDefaultTranslations` is not decoration: reading the
...
describe('Unit: getDefaultTranslations memoization', () => {
  it('reads the filesystem once however many times it is called', async () => {
    const underlying = new MockFileSystem(
      {
        'app/translations/en.yml': 'en:\n  beverage: coffee\n',
      },
      'platformos-vfs:/',
    ...
    const fs: AbstractFileSystem = {
      stat: (uri) => underlying.stat(uri),
      readDirectory: (uri) => underlying.readDirectory(uri),
      readFile: (uri) => {
        reads++;
        return underlying.readFile(uri);
      },
    };

    const getDefaultTranslations = makeGetDefaultTranslations(fs, appWith(fs), ROOT);

    const first = await getDefaultTranslations();
    const readsAfterFirst = reads;
    await getDefaultTranslations();
    await getDefaultTranslations();

    // The control: the first call must actually have read something, so that "1 read"
    // cannot be satisfied by a loader that reads nothing at all.
    expect([first, readsAfterFirst > 0, reads]).toEqual([
      { beverage: 'coffee' },
      true,
      readsAfterFirst,
    ]);
