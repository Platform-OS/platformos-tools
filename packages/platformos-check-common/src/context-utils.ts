...
async function getDefaultTranslations(
  ...
  try {
    ...
    // `json: true` for the duplicated mapping key, which strict js-yaml rejects the whole
    // document for — see `TranslationProvider.loadYaml` for what reading a translation
    // file as empty costs.
    const data = load(yamlContent, { json: true }) as Record<string, any>;
