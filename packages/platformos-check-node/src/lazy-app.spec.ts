...
import { Parser } from '@platformos/platformos-common';

import { appCheckRun, getApp, loadConfig, nodeParsers, Offense } from './index';
import { Tree, Workspace, lintBufferOffenses, makeTempWorkspace } from './test/test-helpers';

/**
 * Pins TASK-12.6's central claim: `getApp` no longer reads or parses the project,
 * and a `lintBuffer` call pays only for the file it visits plus the handful of
 * files that file actually points at.
 *
 * Parse counts are the assertion because they are the cost. Before the App model
 * `getApp` parsed every file on every call — 3.6-5.8 s on a 1400-file project, all
 * of it discarded, and the source of the 400-650 MB RSS peaks.
 */
describe('lazy app loading', () => {
  ...
  it('lintBuffer matches the whole-project run filtered to the same file', async () => {
    ...
    const configPath = path.join(root, '.platformos-check.yml');
    const homePath = path.join(root, 'app/views/pages/home.liquid');
    const homeUri = URI.file(homePath).toString().replace(/\\/g, '/');
 ...
 */
async function withCountedLiquidParses<T>(
  lint: () => Promise<T>,
