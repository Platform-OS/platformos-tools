 ...
 * `RouteTable.build()` gets by reading EVERY page's frontmatter — ~1000 `readFile`
 * and ~170 ms on a 1400-file project. That cost is reads, not ASTs, so lazy
 * parsing does nothing for it, and check-node used to pay it on every single lint
 * run because it passed no existing table. (The language server has always passed
 * a persistent one.)
 *
 * Holding one per process is what removes it. The fingerprints are what make that
 * safe: a long-lived process must not answer from a table built before the agent
 * it is serving started editing files.
 */
...
/**
 * Recorded for a page whose current state cannot be established — an unsaved
 * buffer, or a filesystem that reports no modification time. It never equals a
 * real fingerprint, so such a page is re-read on the next run rather than trusted.
 */
...
export async function getSharedRouteTable(rootUri: UriString, app: App): Promise<RouteTable> {
  ...
  if (!table.isBuilt()) {
    const entries = await Promise.all(pages.map(readPage));
  ...
  await Promise.all(
    pages.map(async (page) => {
      const current = await fingerprintOf(page);
...
/**
 * Build the shared table from disk without an app in hand, for callers that want
 * routes before a lint run — e.g. warming a server at startup instead of on its
 * first request.
 */
export async function warmRouteTable(rootUri: UriString): Promise<RouteTable> {
  if (shared?.rootUri !== rootUri) {
    shared = { rootUri, table: new RouteTable(NodeFileSystem), fingerprints: new Map() };
  }
  if (!shared.table.isBuilt()) await shared.table.build(URI.parse(rootUri));
  return shared.table;
}

/** A page's content plus the fingerprint it had when read, or `undefined` if unreadable. */
async function readPage(
  ...
  try {
    ...
    return { uri: page.uri, content: await NodeFileSystem.readFile(page.uri), fingerprint };
  } catch {
...
async function fingerprintOf(page: AppFile): Promise<string> {
  if (page.version !== undefined) return UNKNOWN;
  try {
    const stat = await NodeFileSystem.stat(page.uri);
