 ...
 *
 * That is the entire exclusion mechanism. There is no list of extensions to skip
 * anywhere in the toolchain, and there must not be one: an ignore-list lives in
 * whichever consumer remembered to consult it, and the one we had — a
 * `/\.(s?css|js)\.liquid$/` test inside `isSupportedSourceFile` — was consulted by
 * the language server and not by the lint, so the CLI parsed `theme.css.liquid` as
 * Liquid+HTML and reported `LiquidHTMLSyntaxError` on files the editor would not
 * even open.
 *
 * Keyed by {@link sourceKeyOf}, not by the bare extension, because a `.liquid`
 * file's body language is its RESPONSE FORMAT: `users.json.liquid` is a Liquid
 * template producing JSON, `theme.css.liquid` one producing CSS. The `.liquid`
 * suffix in each key is what keeps that separate from a plain `.json` or `.css`
 * file, which is not a platformOS source at all.
 *
 * `css` and `js` are the two entries of the platform's twelve-value FORMAT_ENUM
 * (`app/models/concerns/custom_view.rb:9`) with no row: their body is a stylesheet
 * or a script, and the Liquid+HTML parser reads the `<` in `a < b` as a tag. They
 * are absent, not excluded — the difference is that absence cannot be forgotten by
 * a consumer.
 *
 * `.json` has no row either, for the reason it never did: a platformOS app has no
 * JSON source type. JSON responses come from `.json.liquid`, and the only `.json`
 * files the platform deploys are generated manifests no check looks at. Ruby's
 ...
 * `.css.liquid` partial.
 *
 * `undefined` is the whole of "do not parse this". An {@link AppFile} with no type
 * never reaches a parser, so no consumer needs to know the reason, and a check that
 * narrows on `SourceCodeType` cannot be handed a file it cannot read.
 */
