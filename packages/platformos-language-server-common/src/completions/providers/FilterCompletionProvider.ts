...
import { FilterEntry, Parameter } from '@platformos/platformos-check-common';
import {
  CompletionItem,
  CompletionItemKind,
  InsertTextFormat,
  ...
  options: (inputType: PseudoType) => Promise<MaybeDeprioritisedFilterEntry[]> = memoize(
    async (inputType) => {
      ...
      const options = filterEntries
        .filter(
          (entry) =>
            entry.syntax?.startsWith(inputType) ||
            (inputType !== 'variable' && entry.syntax?.startsWith('untyped')),
        )
        .sort(sortByName);
