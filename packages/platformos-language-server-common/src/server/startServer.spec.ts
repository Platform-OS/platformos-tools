  ...
  describe('circular render detection', () => {
    ...
    it('should publish an error diagnostic when a render cycle is detected', async () => {
      ...
      connection.triggerNotification(DidChangeWatchedFilesNotification.type, {
        changes: [
          {
            uri: path.join(mockRoot, 'app/views/partials/a.liquid'),
            type: FileChangeType.Created,
          },
          {
            uri: path.join(mockRoot, 'app/views/partials/b.liquid'),
            type: FileChangeType.Created,
          },
          {
            uri: path.join(mockRoot, 'app/views/pages/index.liquid'),
            type: FileChangeType.Created,
          },
        ],
      });
      ...
      const diagCalls = connection.spies.sendNotification.mock.calls.filter(
        (call: any[]) => call[0] === PublishDiagnosticsNotification.method,
      );
      const cycleDiag = diagCalls.find((call: any[]) =>
        call[1]?.diagnostics?.some((d: any) => d.message?.includes('Circular render')),
      );
      expect(cycleDiag).toBeDefined();
      expect(cycleDiag![1].uri).toMatch(/[ab]\.liquid/);
    });
    ...
    it('should clear cycle diagnostics when the cycle is resolved', async () => {
      ...
      const diagCallsBefore = connection.spies.sendNotification.mock.calls.filter(
        (call: any[]) => call[0] === PublishDiagnosticsNotification.method,
      );
      const cycleDiag = diagCallsBefore.find((call: any[]) =>
        call[1]?.diagnostics?.some((d: any) => d.message?.includes('Circular render')),
      );
      expect(cycleDiag).toBeDefined();

      // Now resolve the cycle: b.liquid no longer renders a
      connection.spies.sendNotification.mockClear();
      fileTree['app/views/partials/b.liquid'] = `{# no render #}`;
      connection.triggerNotification(DidChangeWatchedFilesNotification.type, {
        changes: [
          {
            uri: path.join(mockRoot, 'app/views/partials/b.liquid'),
      ...
      // Assert that previously affected URIs now receive empty diagnostics
      const clearCalls = connection.spies.sendNotification.mock.calls.filter(
        (call: any[]) =>
          call[0] === PublishDiagnosticsNotification.method &&
          Array.isArray(call[1]?.diagnostics) &&
          call[1].diagnostics.length === 0 &&
          /[ab]\.liquid/.test(call[1].uri),
      );
