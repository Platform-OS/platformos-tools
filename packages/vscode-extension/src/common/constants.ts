 ...
 *
 * There are no `json`/`jsonc` entries: platformOS serves JSON from `.json.liquid`, so a
 * `.json` file is an asset rather than a source. Nothing could be served through them
 * anyway — `JSONLanguageService` needs `jsonValidationSet.schemas()`, and the platformOS
 * docset returns `[]` for it.
 */
